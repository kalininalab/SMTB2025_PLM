...
}).to_csv(f"/srv/scratch/PLM/embeddings/esm_t{n}/{name}/layer_{args.layer}/metrics_linreg.csv", index=False)
...
}).to_csv(f"/srv/scratch/PLM/embeddings/esm_t{n}/{name}/layer_{args.layer}/predictions_linreg.csv", index=False)


'''if not os.path.exists("/srv/scratch/PLM/results_linreg.csv"):
    with open("/srv/scratch/PLM/results.csv", "w") as f:
        f.write("Model Name, #layers, Dataset, Spearman, Pearson\n")  # Create a table

with open("/srv/scratch/PLM/results.csv", "a") as f:
    f.write(f"{args.esm_model}, {args.layer}, {args.dataset}, {spearman}, {pearson}\n")'''
